...
├── video_source.py        # 🎬 비디오 파일 관리 (카메라 대체) 🆕
...
**역할**:
- `camera.py`: CSI 카메라에서 영상 받아오기 (백그라운드 스레드, 운영용)
- `video_source.py`: 비디오 파일에서 프레임 읽기 (camera.py와 동일 인터페이스, 테스트용)
...
```
test/
├── run_video.py          # 🎬 비디오 파일로 검출 파이프라인 테스트
├── README.md             # 📖 테스트 모드 사용법
└── (*.mp4)               # 테스트 영상 파일 (git 미추적)
```

**역할**:
- 카메라 없이 녹화 영상으로 전체 검출 파이프라인 테스트
- `VideoFileManager`로 `CameraManager`를 대체 (동일 인터페이스)
- 검출/추적/ROI/DynamoDB/웹서버 모두 동일하게 동작
...
| 파일 | 수정하는 경우 |
|------|--------------|
| `main.py` | 프로그램 전체 흐름 변경 (카메라 모드) |
| `test/run_video.py` | 비디오 테스트 흐름 변경 |
...
### "카메라 없이 테스트하고 싶어요" 🔧
→ `test/run_video.py` + `src/core/video_source.py` 사용
